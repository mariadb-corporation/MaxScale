...
*/
    ...
    printf("Stopping %d\n", 0); fflush(stdout);
    sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s'", Test->galera->sshkey[0], Test->galera->access_user, Test->galera->IP[0], Test->galera->access_sudo, Test->galera->stop_db_command);
    printf("%s\n", sys1);  fflush(stdout);
    system(sys1); fflush(stdout);
    ...
    for (i = 1; i < Test->galera->N; i++) {
        printf("Stopping %d\n", i); fflush(stdout);
        sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  %s@%s '%s %s'", Test->galera->sshkey[i], Test->galera->access_user, Test->galera->IP[i], Test->galera->access_sudo, Test->galera->stop_db_command);
        printf("%s\n", sys1);  fflush(stdout);
        system(sys1); fflush(stdout);
    }
    ...
    printf("Starting back all Galera nodes\n");  fflush(stdout);
    printf("Starting node %d\n", Test->galera->N-2); fflush(stdout);
    sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s --wsrep-cluster-address=gcomm://'", Test->galera->sshkey[0],  Test->galera->access_user, Test->galera->IP[0], Test->galera->access_sudo, Test->galera->start_db_command);
    printf("%s\n", sys1);  fflush(stdout);
    system(sys1); fflush(stdout);

    for (i = 1; i < Test->galera->N; i++) {
        printf("Starting node %d\n", i); fflush(stdout);
        sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s --wsrep-cluster-address=gcomm://%s'", Test->galera->sshkey[i],  Test->galera->access_user, Test->galera->IP[i], Test->galera->access_sudo, Test->galera->start_db_command, Test->galera->IP[0]);
        printf("%s\n", sys1);  fflush(stdout);
        system(sys1); fflush(stdout);
    }
