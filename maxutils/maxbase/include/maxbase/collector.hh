 ...
 *  is also passed to make_updates(). This is tun can be used as a Context class.
...
private:
    ...
    {
        /// Misconfigured updater. Either turn off the COLLECTOR_MODE feature or
...
void Collector<SD, COLLECTOR_MODE>::run()
{
    ...
    while (m_running.load(std::memory_order_acquire))
    {
        ...
        if (m_local_queue.empty())
        {
            ...
            if (COLLECTOR_MODE == CollectorMode::NORMAL)
            {
                if (gc_ptr_count)
                {
                    gc_ptr_count = gc();
