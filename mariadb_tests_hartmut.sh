...
ssh -i $maxscale_sshkey -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $access_user@$maxscale_IP "cat /home/$access_user/maxscale-mysqltest/fail.txt" | grep "FAILED"
if [ $? -ne 0 ]; then
        res=0
else 
        res=1
