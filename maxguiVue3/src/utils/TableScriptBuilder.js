...
export default class TableScriptBuilder {
  ...
  buildColDfnSQL({ col, isUpdated }) {
    ...
    const { name, data_type, nn, un, zf, ai, generated, default_exp, charset, collate, comment } =
      colObj
    if (!this.isCreating)
      if (isUpdated) {
  ...
  buildRemovedFkSQL() {
     ...
     */
    const removedKeys = [...foreignKeyDiffs.get('removed'), ...foreignKeyDiffs.get('updated')].map(
      (item) => {
