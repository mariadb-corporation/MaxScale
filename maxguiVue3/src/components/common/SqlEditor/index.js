...
import '@/components/common/SqlEditor/style.scss'
  ...
  computed: {
    ...
    additionalCmplItems() {
      ...
      for (const item of dist) {
        if (nodeTypes.includes(item.type)) item.kind = this.monaco.languages.CompletionItemKind.Text
  ...
  methods: {
    ...
    addWatchers(editor) {
      ...
      editor.onDidChangeModelContent((event) => {
        ...
        if (this.modelValue !== editorValue) this.$emit('update:modelValue', editorValue, event)
      })
      let prevSelectedTxt = ''
      ...
      editor.onDidChangeConfiguration(() => {
        ...
        if (tabFocusMode !== this.isTabMoveFocus) this.$emit('update:isTabMoveFocus', tabFocusMode)
      })
    },
