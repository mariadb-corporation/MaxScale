...
export default {
  ...
  actions: {
    ...
    async checkAsyncCmdRes({ dispatch }, param) {
      try {
        const { monitorModule, monitorId, successCb, showSnackbar, custAsyncCmdDone } = param
        ...
        if (status === 200) {
          ...
          else if (this.vue.$typy(custAsyncCmdDone).isFunction) await custAsyncCmdDone(meta)
    ...
    async handleAsyncCmdPending({ commit, dispatch }, param) {
      ...
      if (isRunning && !isCancelled) {
        ...
        if (showSnackbar)
          commit('mxsApp/SET_SNACK_BAR_MESSAGE', { text: errArr, type: 'error' }, { root: true })
