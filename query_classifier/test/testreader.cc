...
TestReader::TestReader(istream& in, size_t line)
    : m_in(in)
    , m_line(line)
    , m_delimiter(";")
{
    ...
    while (!error && !found && std::getline(m_in, line))
    {
        ...
        if (!line.empty() && (line.at(0) != '#'))
        {
            ...
            if (!skip)
            {
                ...
                string::iterator i
                    = std::find_if(line.begin(), line.end(), std::ptr_fun<int, int>(std::isspace));
            ...
            if (i != string::npos)
            {
                ...
                if ((line.find("-- ", i) != string::npos) || (line.find("#", i) != string::npos))
                {
