...
int main(int argc, char* argv[])
{
    ...
    int baseline = test->maxscales->ssh_node_f(0, false, "maxscale -c -f /tmp/maxscale.cnf");

    /** Configure bad parameter for a listener */
    test->maxscales->ssh_node_f(0, true, "sed -i -e 's/service/ecivres/' /tmp/maxscale.cnf");
    test->add_result(baseline == test->maxscales->ssh_node_f(0, false, "maxscale -c -f /tmp/maxscale.cnf"),
