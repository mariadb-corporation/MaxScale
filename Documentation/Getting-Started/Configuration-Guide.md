...
validate that the token was indeed generated by the correct entity. When
combined with the `admin_jwt_extra_certs` parameter, it allows multiple
MaxScales to share authentication tokens.
...
- **Type**: [pathlist](#path-lists)
- **Mandatory**: No
- **Dynamic**: No
- **Default**: `""`

A list of paths to extra public certificates. Any JWT signed by one of these
certificates is accepted as a valid token for the REST API. The user (i.e. value
of the `"sub"` field) must exist on the MaxScale instance for the login to be
accepted.

For example, when configured with the public certificates of other MaxScale
instances, this allows multiple MaxScales to share authentication tokens with
each other without having to perform authentication on all instance. This is
especially useful in situations where the MaxScale REST API is behind a proxy of
some sorts.
