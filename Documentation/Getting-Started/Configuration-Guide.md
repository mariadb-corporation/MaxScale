...
Starting with MaxScale 7, the `Maintenance` and `Drain` states of servers will
be synchronized. In older versions the server states had to be manually assigned
on each MaxScale instance.

* ([MXS-3619](https://jira.mariadb.org/browse/MXS-3619)) The synchronization
  only affects the MaxScale configuration.
