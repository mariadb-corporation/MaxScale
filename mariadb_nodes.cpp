...
int Mariadb_nodes::stop_nodes()
{
    ...
    for (i = 0; i < N; i++) {
        ...
        printf("Stopping %d\n", i); fflush(stdout);
        sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s'", sshkey[i], access_user, IP[i], access_sudo, stop_db_command);
        printf("%s\n", sys1);  fflush(stdout);
...
int Mariadb_nodes::start_replication()
{
    ...
    sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s'", sshkey[0], access_user, IP_private[0], access_sudo, start_db_command);
    printf("%s\n", sys1);  fflush(stdout);
    global_result +=  system(sys1); fflush(stdout);

    for (i = 1; i < N; i++) {
        printf("Starting node %d\n", i); fflush(stdout);
        sprintf(&sys1[0], "ssh -i %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null %s@%s '%s %s '", sshkey[i], access_user, IP_private[i], access_sudo, start_db_command);
        printf("%s\n", sys1);  fflush(stdout);
        global_result += system(sys1); fflush(stdout);
    }
    sleep(5);
...
int Mariadb_nodes::set_slave(int node, char master_host[], int master_port, char log_file[], char log_pos[])
{
    char str[1024];
