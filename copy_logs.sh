...
ssh -i $maxscale_sshkey -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $access_user@$maxscale_IP "rm -rf logs; mkdir logs; $access_sudo cp $maxscale_log_dir/*.log logs/; $access_sudo cp /tmp/core* logs; $access_sudo chmod 777 -R logs"
