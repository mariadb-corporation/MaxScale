...
int main(int argc, char* argv[])
{
    ...
    auto slave = get_row(test.maxscale->conn_rwsplit[0], "SELECT @@server_id");
    test.try_query(test.maxscale->conn_rwsplit[0], "BEGIN");
    auto master = get_row(test.maxscale->conn_rwsplit[0], "SELECT @@server_id");
    ...
    test.expect(execute_query_silent(test.maxscale->conn_rwsplit[0], "SELECT @@last_insert_id") != 0,
    ...
    auto backup_slave = get_row(test.maxscale->conn_rwsplit[0], "SELECT @@server_id");
