...
void test_rebuild_custom_datadir(TestConnections& test, int target_ind, int master_ind);
...
void test_main(TestConnections& test)
{
    ...
    if (test.ok())
    {
        ...
        if (test.ok())
        {
            test_rebuild_custom_datadir(test, target_ind, master_ind);
        }
...
void test_rebuild_custom_datadir(TestConnections& test, int target_ind, int master_ind)
{
    ...
    auto data = mxs.get_servers();
    data.print();
    auto target_gtid = data.get(target_ind).gtid;
    auto source_gtid = data.get(master_ind).gtid;
    ...
    if (test.ok())
    {
        ...
        run_rebuild(test, cmd, target_ind, master_ind);
        if (test.ok())
        {
