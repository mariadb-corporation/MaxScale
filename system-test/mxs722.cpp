...
int main(int argc, char* argv[])
{
    ...
    test->maxscale->ssh_node_f(0, true, "cp /etc/maxscale.cnf /tmp/maxscale.cnf");
    test->maxscale->ssh_node_f(0, true, "chmod a+rw /tmp/maxscale.cnf");

    /** Get a baseline result with a good configuration */
    int baseline = test->maxscale->ssh_node_f(0, true, "maxscale -c --user=maxscale -f /tmp/maxscale.cnf");

    /** Configure bad parameter for a listener */
    test->maxscale->ssh_node_f(0, true, "sed -i -e 's/service/ecivres/' /tmp/maxscale.cnf");
    test->add_result(baseline == test->maxscale->ssh_node_f(0, true, "maxscale -c --user=maxscale -f /tmp/maxscale.cnf"),
    ...
    test->maxscale->ssh_node_f(0, true, "sed -i -e 's/filebase/basefile/' /tmp/maxscale.cnf");
    test->add_result(baseline == test->maxscale->ssh_node_f(0, true, "maxscale -c --user=maxscale -f /tmp/maxscale.cnf"),
                     "Bad filter parameter should be detected.\n");

    /** Remove configuration file */
    test->maxscale->ssh_node_f(0, true, "rm -f /tmp/maxscale.cnf");
    test->add_result(baseline == test->maxscale->ssh_node_f(0, true, "maxscale -c --user=maxscale -f /tmp/maxscale.cnf"),
