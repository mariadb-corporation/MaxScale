    ...
    for (i = 1; i <= N; i++)
    {
        ...
        if (file)
        {
            ...
            while (!feof(file))
            {
                ...
                if (execute_query_from_file(Test->maxscale->conn_rwsplit, file) == 1)
                {
        ...
        if (file)
        {
            ...
            while (!feof(file))
            {
                ...
                int rc = execute_query_from_file(Test->maxscale->conn_rwsplit, file);

                if (rc != -1 && (rc == 0
                                 || mysql_errno(Test->maxscale->conn_rwsplit) != 1141))
                {
