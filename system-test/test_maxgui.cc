...
int main(int argc, char** argv)
{
    ...
    auto res = test.maxscales->ssh_output("test -d /usr/share/maxscale/gui");
    test.expect(res.rc == 0, "GUI files not found at: /usr/share/maxscale/gui/");

    res = test.maxscales->ssh_output("curl -s -f localhost:8989/index.html");

    test.expect(res.rc == 0, "Root resource should serve the GUI main page.");
    test.expect(res.output.find("src=/js/") != std::string::npos,
