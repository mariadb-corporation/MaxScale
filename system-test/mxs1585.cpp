...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int x = 1; x <= 2; x++)
        {
            test.maxscales->ssh_node_f(0, true, "maxctrl set server server%d maintenance", x);
            sleep(1);
            test.maxscales->ssh_node_f(0, true, "maxctrl clear server server%d maintenance", x);
            sleep(2);
        }
    }
