...
int install_kerberos(TestConnections& test, std::string machine_name, std::string maria_version)
{
    int res = system(("mdbci install_product --product kerberos_server " + machine_name).c_str());
    test.reset_timeout();
    // Ignoring exit code becase in some versions of MariaDB gssapi is included into client/server
    system(("mdbci install_product --product plugin_gssapi_client --product-version " + maria_version + " "
            + machine_name).c_str());
    test.reset_timeout();
    system(("mdbci install_product --product plugin_gssapi_server --product-version " + maria_version + " "
            + machine_name).c_str());
    test.reset_timeout();
    return res;
}
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < Test->repl->N; i++)
    {
        ...
        install_kerberos(*Test, machine_name, maria_vrs);
