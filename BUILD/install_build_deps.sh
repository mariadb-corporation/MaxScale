...
then
  ...
  ${apt_cmd} install dpkg-dev git wget cmake \
       ...
       libmicrohttpd-dev libboost-all-dev librdkafka-dev libmemcached-dev libhiredis-dev unixodbc-dev libssh-dev
...
then
    ...
    sudo zypper -n install gcc gcc-c++ cmake ncurses-devel bison glibc-devel libgcc_s1 perl \
         ...
         libmicrohttpd-devel boost-devel librdkafka-devel libmemcached-devel unixODBC-devel libssh-devel
