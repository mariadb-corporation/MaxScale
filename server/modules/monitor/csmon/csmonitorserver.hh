...
public:
    ...
    CsMonitorServer(SERVER* pServer, const SharedSettings& shared, CsContext* pCs_context);
    ...
    public:
        ...
        bool ok() const { return response.is_success() && sJson; }
    ...
    public:
        ...
        bool ok() const { return Result::ok() && sXml; }
    ...
    const char* name() const { return this->server->name(); }

    const char* address() const { return this->server->address(); }

    cs::Version minor_version() const { return m_minor_version; }

    int version_number() const { return m_version_number; }
    ...
    void set_status(uint64_t bit) { this->server->set_status(bit); }
    ...
    NodeMode node_mode() const { return m_node_mode; }

    void set_node_mode(NodeMode node_mode) { m_node_mode = node_mode; }

    bool set_node_mode(const Config& config, json_t* pOutput);

    bool is_multi_node() const { return m_node_mode == MULTI_NODE; }

    bool is_single_node() const { return m_node_mode == SINGLE_NODE; }

    bool is_unknown_mode() const { return m_node_mode == UNKNOWN_MODE; }
    ...
    TrxState trx_state() const { return m_trx_state; }

    bool in_trx() const { return m_trx_state == TRX_ACTIVE; }
    ...
    bool set_cluster_mode(
        cs::ClusterMode mode, const std::chrono::seconds& timeout, json_t* pOutput = nullptr);

    static Result fetch_status(const std::vector<CsMonitorServer*>& servers, CsContext& context);
    static Statuses fetch_statuses(const std::vector<CsMonitorServer*>& servers, CsContext& context);
    static bool fetch_statuses(
        const std::vector<CsMonitorServer*>& servers, CsContext& context, Statuses* pStatuses);

    static Result fetch_config(const std::vector<CsMonitorServer*>& servers, CsContext& context);
    static Configs fetch_configs(const std::vector<CsMonitorServer*>& servers, CsContext& context);
    static bool fetch_configs(
    ...
    static Result remove_node(const std::vector<CsMonitorServer*>& servers,
        ...
        const std::chrono::seconds& timeout,
        CsContext& context);
    static Results rollback(const std::vector<CsMonitorServer*>& servers, CsContext& context);
    static bool rollback(const std::vector<CsMonitorServer*>& servers, CsContext& context, Results* pResults);
    ...
    static CsMonitorServer* get_master(
        const std::vector<CsMonitorServer*>& servers, CsContext& context, json_t* pOutput = nullptr);

private:
    bool set_status(const mxb::http::Response& response, json_t** ppError);

    std::string create_url(
        cs::rest::Scope scope, cs::rest::Action action, const std::string& tail = std::string()) const;
