...
public:
    ...
    const CsContext& context() const { return m_context; }
    ...
    const ServerVector& servers() const { return reinterpret_cast<const ServerVector&>(Base::servers()); }
    ...
    bool command_remove_node(json_t** ppOutput, const std::string& host, const std::chrono::seconds& timeout);
...
private:
    ...
    void cs_add_node(json_t** ppOutput,
        mxb::Semaphore* pSem,
        const std::string& host,
        const std::chrono::seconds& timeout);
    void cs_config_get(json_t** ppOutput, mxb::Semaphore* pSem, CsMonitorServer* pServer);
    void cs_mode_set(
        json_t** ppOuput, mxb::Semaphore* pSem, cs::ClusterMode mode, const std::chrono::seconds& timeout);
    void cs_remove_node(json_t** ppOutput,
        mxb::Semaphore* pSem,
        const std::string& host,
        const std::chrono::seconds& timeout);
...
#if defined(CSMON_EXPOSE_TRANSACTIONS)
    void cs_begin(json_t** ppOutput,
        mxb::Semaphore* pSem,
        const std::chrono::seconds& timeout,
        CsMonitorServer* pServer);
    void cs_commit(json_t** ppOutput,
        mxb::Semaphore* pSem,
        const std::chrono::seconds& timeout,
        CsMonitorServer* pServer);
