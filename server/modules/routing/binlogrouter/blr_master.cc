...
static void* CreateMySQLAuthData(const char* username, const char* password, const char* database)
{
    ...
    auth_info->auth_token_phase2.resize(SHA_DIGEST_LENGTH);
    gw_sha1_str((const uint8_t*)password, strlen(password), auth_info->auth_token_phase2.data());
...
bool blr_send_packet(ROUTER_SLAVE* slave, uint8_t* buf, uint32_t len, bool first)
{
    ...
    if (buffer)
    {
        ...
        *data++ = slave->seqno++;
        ...
        mxs_route_reply(slave->up, buffer, nullptr);
    }
    else
    {
