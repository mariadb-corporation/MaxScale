...
struct DebugHandler : public pinloki::parser::Handler
{
    ...
    void start_slave() override { result << "START SLAVE"; }

    void stop_slave() override { result << "STOP SLAVE"; }

    void reset_slave() override { result << "RESET SLAVE"; }
    ...
    void show_master_status() override { result << "SHOW MASTER STATUS"; }

    void show_binlogs() override { result << "SHOW BINARY LOGS"; }
    ...
    void purge_logs(const std::string& up_to) override { result << "PURGE BINARY LOGS TO " << up_to; }

    void error(const std::string& err) override { result << "ERROR"; }
} handler;
