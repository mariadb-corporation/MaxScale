...
bool SmartRouterSession::routeQuery(GWBUF&& buffer)
{
    ...
    {
        auto route_info = m_qc.update_route_info(mariadb::QueryClassifier::CURRENT_TARGET_UNDEFINED, &buffer);
        std::string canonical = buffer.get_canonical();
