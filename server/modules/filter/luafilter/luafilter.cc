                                      ...
                                      mxs::Upstream* upstream)
{
    ...
    if (my_instance->session_script)
    {
        ...
        else
        {
            ...
            lua_pushstring(my_session->lua_state, session->client_dcb->m_user);
            lua_pushstring(my_session->lua_state, session->client_dcb->m_remote);
    ...
    if (my_session && my_instance->global_lua_state)
    {
        ...
        lua_pushstring(my_instance->global_lua_state, session->client_dcb->m_user);
        lua_pushstring(my_instance->global_lua_state, session->client_dcb->m_remote);
...
static void diagnostic(MXS_FILTER* instance, MXS_FILTER_SESSION* fsession, DCB* dcb)
{
    ...
    if (my_instance)
    {
        if (my_instance->global_lua_state)
        {
            ...
            {
                dcb_printf(dcb,
                           "Global scope call to 'diagnostic' failed: '%s'.\n",
                           lua_tostring(my_instance->global_lua_state, -1));
                lua_pop(my_instance->global_lua_state, -1);
            }
        }
