...
GWBUF* Backend::create_ok_response()
{
    ...
    const static uint8_t ok[MYSQL_OK_PACKET_MIN_LEN] = {07, 00, 00, 01, 00, 00, 00, 02, 00, 00, 00};
...
public:
    ResultSetDCB(MXS_SESSION* session)
        : ClientDCB(
            DCB::FD_CLOSED, "127.0.0.1", sockaddr_storage {}, DCB::Role::CLIENT, session, nullptr, nullptr)
        , m_protocol(this)
    {}

    GWBUF* create_response() const { return gwbuf_alloc_and_load(m_response.size(), &m_response.front()); }
...
private:
    ...
    public:
        ...
        int32_t write(GWBUF* pBuffer) override { return m_owner.write(pBuffer); }
        ...
        ClientDCB* dcb() override { return &m_owner; }

        const ClientDCB* dcb() const override { return &m_owner; }
