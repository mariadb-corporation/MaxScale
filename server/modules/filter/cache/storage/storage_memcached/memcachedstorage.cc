...
{
public:
    ~MemcachedToken() { memcached_free(m_pMemc); }

    std::shared_ptr<MemcachedToken> get_shared() { return shared_from_this(); }
        ...
        shared_ptr<Storage::Token>* psToken)
    {
        ...
        if (pMemc)
        {
            ...
            {
                MemcachedToken* pToken = new (std::nothrow)
                    MemcachedToken(pMemc, address, port, timeout, soft_ttl, hard_ttl, mcd_ttl);
            ...
            {
                MXS_ERROR("Could not turn on memcached binary protocol: %s", memcached_strerror(pMemc, mrv));
                memcached_free(pMemc);
            }
        }
        else
        ...
        const std::function<void(cache_result_t)>& cb)
    {
        ...
        mxs::thread_pool().execute([sThis, mkey, pClone, cb]() {
            ...
            memcached_return_t mrv = memcached_set(sThis->m_pMemc,
                mkey.data(),
                mkey.size(),
...
MemcachedStorage::MemcachedStorage(
    const string& name, const Config& config, const string& address, int port, uint32_t max_value_size)
    : m_name(name)
    , m_config(config)
    , m_address(address)
    , m_port(port)
...
//static
MemcachedStorage* MemcachedStorage::create(
    const string& name, const Config& config, const std::string& argument_string)
{
    ...
    else
    {
        ...
        if (Storage::split_arguments(argument_string, &arguments))
        {
            ...
            {
                MXS_WARNING(
                    "Unknown `storage_memcached` argument: %s=%s", kv.first.c_str(), kv.second.c_str());
            }
            ...
            if (!error)
            {
                ...
                pStorage = new (std::nothrow)
                    MemcachedStorage(name, config, host.address(), host.port(), max_value_size);
            }
        }
        else
        {
...
{
    return MemcachedToken::create(
        m_address, m_port, m_config.timeout, m_config.soft_ttl, m_config.hard_ttl, m_mcd_ttl, psToken);
}
...
cache_result_t MemcachedStorage::del_value(
...
cache_result_t MemcachedStorage::invalidate(
