...
static const MXS_ENUM_VALUE option_values[]
    = {{"ignorecase", REG_ICASE}, {"case", 0}, {"extended", REG_EXTENDED}, {NULL}};
...
static MXS_FILTER* createInstance(const char* name, mxs::ConfigParameters* params)
{
    ...
    if (my_instance)
    {
        ...
        if (my_instance->exclude && regcomp(&my_instance->exre, my_instance->exclude, cflags))
        {
            MXS_ERROR("Invalid regular expression '%s'"
    ...
    MXS_FILTER* instance, MXS_SESSION* session, SERVICE* service, mxs::Downstream* down, mxs::Upstream* up)
{
    ...
    if ((my_session = static_cast<TOPN_SESSION*>(MXS_CALLOC(1, sizeof(TOPN_SESSION)))) != NULL)
    {
        ...
        sprintf(my_session->filename, "%s.%d", my_instance->filebase, my_instance->sessions);
...
static void closeSession(MXS_FILTER* instance, MXS_FILTER_SESSION* session)
{
    ...
    if ((fp = fopen(my_session->filename, "w")) != NULL)
    {
        ...
        fprintf(fp,
            ...
            (double) ((my_session->total.tv_sec * 1000) + (my_session->total.tv_usec / 1000))
                / (1000 * statements));
        fprintf(fp,
    ...
    const mxs::Reply& reply)
{
    ...
    if (my_session->current)
    {
        ...
        if (inserted)
        {
            qsort(my_session->top, my_instance->topN, sizeof(TOPNQ*), cmp_topn);
        }
        else
        {
            MXS_FREE(my_session->current);
        ...
        for (int i = 0; i < my_instance->topN; i++)
        {
            if (my_session->top[i]->sql)
            {
                double exec_time = ((my_session->top[i]->duration.tv_sec * 1000.0)
                                       + (my_session->top[i]->duration.tv_usec / 1000.0))
                                 / 1000.0;
