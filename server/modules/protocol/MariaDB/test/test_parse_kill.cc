...
int test_one_query(
    const char* query, bool should_succeed, uint64_t expected_tid, int expected_kt, std::string expected_user)
{
    ...
    if (success == should_succeed && result_tid == expected_tid && result_kt == expected_kt
        && expected_user == user)
    {
        return 0;
    }
...
{
    test_t tests[] = {{" kill ConNectioN 123  ", true, 123, KT_CONNECTION},
