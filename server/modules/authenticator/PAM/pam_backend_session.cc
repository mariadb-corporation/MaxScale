...
{
    MYSQL_session* ses = (MYSQL_session*)dcb->session()->client_dcb->protocol_data();
...
bool PamBackendAuthenticator::extract(DCB* dcb, GWBUF* buffer)
{
    ...
    if (unexpected_data)
    {
        MXS_ERROR("Failed to read data from '%s' when authenticating user '%s'.", srv_name, dcb->m_user);
    }
    return success;
}
