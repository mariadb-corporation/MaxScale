...
#define MXB_MODULE_NAME "pp_postgres"
...
const char* progname = program_invocation_name; // The presence of "progname" is assumed by Postgres.
...
/*
 * PostgresProtocolInfo
 */
class PostgresProtocolInfo : public GWBUF::ProtocolInfo
{
public:
    PostgresProtocolInfo(std::string_view sql)
        : m_canonical(make_canonical(sql))
    {
    }

    PostgresProtocolInfo(const PostgresProtocolInfo&) = delete;
    PostgresProtocolInfo& operator=(const PostgresProtocolInfo&) = delete;

    static PostgresProtocolInfo* get(const Parser::Helper& helper,
                                     const GWBUF& query,
                                     uint32_t collect)
    {
        PostgresProtocolInfo* pInfo = nullptr;
        ...
        return static_cast<PostgresProtocolInfo*>(query.get_protocol_info().get());
    }
    ...
    void analyze(std::string_view sql, uint32_t collect)
    {
        ...
        PG_TRY();
        {
            pList = raw_parser(sql.data(), RAW_PARSE_DEFAULT);
...
private:
    ...
    static bool is_query_parsed(const GWBUF& query, uint32_t collect)
    {
        ...
        auto* pInfo = static_cast<PostgresProtocolInfo*>(query.get_protocol_info().get());
    ...
    static void parse_query(const Parser::Helper& helper, const GWBUF& query, uint32_t collect)
    {
        ...
        PostgresProtocolInfo* pInfo = static_cast<PostgresProtocolInfo*>(query.get_protocol_info().get());
        if (pInfo)
        {
        ...
        {
            auto sInfo = std::make_unique<PostgresProtocolInfo>(sql);
            pInfo = sInfo.get();
 ...
 * PostgresParser
 */
class PostgresParser : public mxs::Parser
{
public:
...
private:
    PostgresProtocolInfo* get_info(const GWBUF& query, uint32_t collect_extra = 0) const
    {
...
/*
 * PostgresParserPlugin
 */
class PostgresParserPlugin : public mxs::ParserPlugin
{
public:
...
MXS_MODULE* MXS_CREATE_MODULE()
{
    ...
    static MXS_MODULE info =
    {
        ...
        mxs::ModuleStatus::GA,
        MXS_PARSER_VERSION,
        "Postgres SQL parser using postgres.",
        "V1.0.0",
