...
const NAME_AND_VALUE facilities[] = {{"LOG_AUTH", LOG_AUTH},
    ...
    for_each(begin, end, [&errors, zProperty, from_string, to_string](const NAME_AND_VALUE& item) {
        ...
        if (item.value != -1)
        {
            if (rv)
            {
                ...
                {
                    ++errors;
                    cerr << "error: Wrong " << zProperty << " was returned for " << item.zName << ", "
                         << item.value << " was expected, but " << value << " was returned." << endl;
                }
            }
            else
            {
                ++errors;
                cerr << "error: " << item.zName << " was not recognized as a syslog " << zProperty << "."
                     << endl;
            }
            ...
            {
                ++errors;
                cerr << "error: Code " << item.value << " was converted to " << zName << " although "
                     << item.zName << " was expected." << endl;
            }
        }
        else
        {
...
const NAME_AND_VALUE events[] = {{"authentication_failure", event::AUTHENTICATION_FAILURE}};
    ...
    for_each(begin, end, [&errors](const NAME_AND_VALUE& item) {
        ...
        if (facility != event::DEFAULT_FACILITY)
        {
            ...
            cerr << "error: Default facility for " << event::to_string(id) << " was "
                 ...
                 << log_facility_to_string(event::DEFAULT_FACILITY) << "." << endl;
        }
        ...
        if (level != event::DEFAULT_LEVEL)
        {
            ...
            cerr << "error: Default level for " << event::to_string(id) << " was "
                 << log_level_to_string(level) << " and not " << log_level_to_string(event::DEFAULT_LEVEL)
                 << "." << endl;
        }
        ...
        {
            ++errors;
            cerr << "error: Set level LOG_ALERT as not stored, but was " << log_level_to_string(level) << "."
                 << endl;
        }
    });
...
const CONFIGURATION configurations[] = {{"event.authentication_failure.facility",
                                            "LOG_LOCAL0",
    ...
    {"event.blah.facility",
        ...
        What::FACILITY,
        LOG_LOCAL0},
    {"blah", "LOG_LOCAL0", static_cast<event::id_t>(-1), event::IGNORED, What::IRRELEVANT, LOG_LOCAL0}};
...
int test_event_configuration()
{
    ...
    for_each(begin, end, [&errors](const CONFIGURATION& c) {
        ...
        if (result == c.result)
        {
            if (result == event::ACCEPTED)
            {
                if (c.what == What::FACILITY)
                {
                    ...
                    {
                        ++errors;
                        cerr << "error: Configuration \"" << c.zParameter << "=" << c.zValue
                             << " did not affect the facility in the expected way." << endl;
                    }
                }
                else
                {
                    ...
                    if (level != c.value)
                    {
                        ++errors;
                        cerr << "error: Configuration \"" << c.zParameter << "=" << c.zValue
                             << " did not affect the level in the expected way." << endl;
                    }
                }
            }
        }
        else
        {
            ++errors;
            cerr << "error: Configuration \"" << c.zParameter << "=" << c.zValue
                 << " did not produce the expected result." << endl;
        }
    });
