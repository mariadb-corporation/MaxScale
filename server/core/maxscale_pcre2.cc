...
mxs_pcre2_result_t mxs_pcre2_simple_match(const char* pattern, const char* subject, int options, int* error)
{
    ...
    size_t erroff;
    mxs_pcre2_result_t rval = MXS_PCRE2_ERROR;
    pcre2_code* re = pcre2_compile((PCRE2_SPTR) pattern, PCRE2_ZERO_TERMINATED, options, &err, &erroff, NULL);
    if (re)
    {
        pcre2_match_data* mdata = pcre2_match_data_create_from_pattern(re, NULL);
        if (mdata)
        {
            int rc = pcre2_match(re, (PCRE2_SPTR) subject, PCRE2_ZERO_TERMINATED, 0, 0, mdata, NULL);
    ...
    int errorcode, const char* module_name, const char* filename, int line_num, const char* func_name)
{
    ...
    if (mxs_log_is_priority_enabled(LOG_ERR))
    {
        ...
        mxs_log_message(
            LOG_ERR, module_name, filename, line_num, func_name, "PCRE2 Error message: '%s'.", errorbuf);
    }
}
